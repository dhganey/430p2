...
// 
// Save this program in "inc.cc", then compile using:
...
sem_t mutex;		// global semaphore, used for mutual exclusion
...
increment( void *arg )
{
	...
	{
		sem_wait( &mutex ); // semaphore WAIT operation
		counter++;
		sem_post( &mutex ); // semaphore SIGNAL operation
	}
